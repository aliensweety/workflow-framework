...
- `--text` 或 `--text-file` (二选一必填)：配音文本，前者直接传字符串、后者从本地文件 utf8 读取
- `--task-id` (必填，get_tts)：start_tts 返回的 task_id
- `--output` (可选，get_tts)：结果音频保存路径。**调用方传入的后缀会被忽略**，实际写盘后缀按 backend 真实容器格式（始终是 `.flac`）。例如传 `voiceover.mp3` 会写成 `voiceover.flac`。返回结果里 `result.file_path` 是实际写盘路径
...
- `参考音频上传失败：未捕获到 /upload/image 响应`：上传请求超时（60s 内未回响应）或失败
- `参考音频上传响应中没有 name 字段`：RunningHub 上传 API 返回结构变化
- `textarea 内容与 --text 不一致`：文本写入后被 React state 还原成旧值（不会再静默用错文本提交）
