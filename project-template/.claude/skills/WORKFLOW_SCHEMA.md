...
本框架有两种 YAML 文件：
- **Workflow YAML**（模板）：`workflows/<n>.yaml`，定义流程，不变。
...
workflow: <string>                   # 必填，对应的 workflow name
...
- `run_id`, `workflow`, `instance`, `created_at`, `status`, `current_step`, `steps_status`
